...
//		$modelFilter = $filter->getModelFilter();
    ...
    protected function getSystemStats() {
		...
		protected function updateUploadedFileInInvitation($invitacion,$file=null) {
			if ($file == null)
				$file = $invitacion->getArchivoAdjunto();

			$user = $this->getJYM()->getLoggedInUser(); //sera un administrador
			$uploader = $user->getApellido() . '_' . $user->getNombre();
			$today = new \DateTime();
			$filename = $invitacion->getInstanciaEvento()->getEvento()->getTitulo(). '_' .$uploader . $today->format('Ymd_His') . '.' .$file->guessExtension();
